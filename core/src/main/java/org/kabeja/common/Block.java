...
public class Block {
   ...
   *
   * <p>This method optionally includes the bounds of entities on the DEFPOINTS layer. According to
   * AutoCAD documentation, entities on the DEFPOINTS layer are not visible when plotting/printing a
   * drawing since the layer is intended to only include dimension information.
   *
   * @param includeDefPointsLayer Whether or not entities on the DEFPOINTS layer should be included
  ...
  private Bounds calculateBoundsFromEntities(boolean includeDefPointsLayer) {
    ...
    if (this.entities.size() > 0) {
      for (DraftEntity entity : this.entities) {
        if (!includeDefPointsLayer
            && entity.getLayer() != null
            && "defpoints".equalsIgnoreCase(entity.getLayer().getName())) {
          continue;
        }
