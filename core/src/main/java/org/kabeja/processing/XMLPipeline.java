...
public class XMLPipeline implements Generator {
  ...
  public void generate(DraftDocument doc, Map<String, Object> context, OutputStream out)
    ...
    this.serializer.setProperties(
        new MergeMap<>(oldProbs, new MergeMap<>(this.serializerProperties, context)));
    ...
    for (int x = 0; x < saxFilterProperties.size(); x++) {
      ...
      this.manager
          .getSAXFilter(sc.getFilterName())
          .setProperties((Map<String, Object>) saxFilterProperties.get(x));
    }
  }
