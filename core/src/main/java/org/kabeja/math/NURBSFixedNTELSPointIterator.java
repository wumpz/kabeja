...
public class NURBSFixedNTELSPointIterator implements Iterator<Point3D> {
  ...
  public boolean hasNext() {
    if (this.t < this.nurbs.getKnots()[this.interval]) {
      ...
      // Make one last point at the end node. This could lead to duplicating the last point, but
      // that's better
      // than missing it.
      onLastNode = true;
  ...
  protected void nextInterval() {
    ...
    // To prevent numerical issues, we want to make sure that we definitely change t when
    // calculating t=t+dt.
    double maxUlp =
        Math.max(
            Math.ulp(nurbs.getKnots()[this.interval]),
