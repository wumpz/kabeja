...
public abstract class AbstractSAXParser extends DefaultHandler implements Parser {
  ...
  public void addHandler(XMLHandler xmlHandler) {
    ...
    if (xmlHandler.getNamespaceHandle().length() == 0) {
      ...
      elementHandlers =
          (Map<String, XMLHandler>) this.nsHandlers.get(xmlHandler.getNamespaceHandle());
    } else {
      elementHandlers = new HashMap<>();
      this.nsHandlers.put(xmlHandler.getNamespaceHandle(), elementHandlers);
    }
   ...
   */
  @Override
  public void parse(InputStream input, DraftDocument doc, Map<String, Object> properties)
   ...
   */
  @Override
  public DraftDocument parse(InputStream in, Map<String, Object> properties) throws ParseException {
  ...
  public void endElement(String uri, String localName, String name) throws SAXException {
    if (this.captureEvent) {
      ...
      if (this.currentHandler.getNamespaceHandle().equals(uri)
          && this.currentHandler.getElementNameHandle().equals(localName)) {
  ...
  protected void fireStartParseElement(String namespace, String name, Attributes attributes)
      throws SAXException {
    this.throughXMLHandler = true;
    this.captureEvent =
        this.currentHandler.startParseElement(namespace, name, attributes, this.context);
  }
