...
public abstract class Entity implements DraftEntity {
  ...
  protected static final int BOOLEAN_BIT_VISIBILITY = 0;
  protected static final int BOOLEAN_BIT_BLOCKENTITY = 1;
  protected static final int BOOLEAN_BIT_MODELSPACE = 2;
  protected static final int BOOLEAN_BIT_OWN_LINETYPE = 3;
  ...
  /**
   * @param visible The visibile to set.
   */
  @Override
  public void setVisibile(boolean visible) {
    this.entityFlags = Utils.setBit(this.entityFlags, BOOLEAN_BIT_VISIBILITY, visible);
  }
  ...
  public void setBlockEntity(boolean b) {
    this.entityFlags = Utils.setBit(this.entityFlags, BOOLEAN_BIT_BLOCKENTITY, b);
  }
  ...
  public void setLinetypeScaleFactor(double linetypeScaleFactor) {
    this.lazyContainer.set(linetypeScaleFactor, LAZY_INDEX_LINETYPE_SCALE);
  }
  ...
  public void setColor(int color) {
    if (this.lazyContainer.contains(LAZY_INDEX_COLOR) || color != Constants.COLOR_VALUE_BY_LAYER) {
      this.lazyContainer.set(new byte[] {(byte) color}, LAZY_INDEX_COLOR);
    }
  }
  ...
  public void setColorRGB(byte[] colorRGB) {
    if (this.lazyContainer.contains(LAZY_INDEX_COLOR) || colorRGB.length > 0) {
  ...
  public void setLineWeight(int lineWeight) {
    if (this.lazyContainer.contains(LAZY_INDEX_LINETYPE) || lineWeight != 0) {
      this.lazyContainer.set(lineWeight, LAZY_INDEX_LINEWEIGHT);
    }
  }
  ...
  public void setTransparency(double transparency) {
    if (this.lazyContainer.contains(LAZY_INDEX_TRANSPARENCY) || transparency != 0.0) {
      this.lazyContainer.set(transparency, LAZY_INDEX_TRANSPARENCY);
    }
  }
  ...
  public void setThickness(double thickness) {
    if (this.lazyContainer.contains(LAZY_INDEX_THICKNESS) || thickness != 0.0) {
  ...
  public void setModelSpace(boolean modelSpace) {
    this.entityFlags = Utils.setBit(this.entityFlags, BOOLEAN_BIT_MODELSPACE, modelSpace);
  }
  ...
  /**
   * Ensures that the Entity's coordinates are in WCS rather than OCS. Should only ever be called on
   * completely built entities, i.e. after the entity's group has benn parsed entirely.
   *
   * <p>Overriding this method is necessary for planar entities (circle, arc, solid, trace, text,
   * attrib, attdef, shape, insert, 2Dpolyline, 2D vertex, lwpolyline, hatch, image), since their
   * coordinates get stored in the OCS.
   *
   * <p>In this case the method does not simply translate the coordinates, but "reverts" the
   * arbitrary axis algorithm's effects. This means that an Entity with Extrusion != (0,0,1), that
   * might have appeared "flipped" without the transformation, will be "unflipped".
   *
   * <p>3D-Entities (line, point, 3dface, 3Dpolyline, 3D mesh, 3D meshvertex, Ellipse) do not need
