...
public class LWPolylineSegment {
  ...
  protected void createCurvedTrapezium(
      Point3D startPoint, Point3D endPoint, double radius, double length) {
    // first get the center point of the arc

    // middle point of chord
    double s = length / 2;
    Vector edgeDirection = MathUtils.getVector(startPoint, endPoint);
    edgeDirection = MathUtils.normalize(edgeDirection);

    Point3D mp = MathUtils.getPointOfStraightLine(startPoint, edgeDirection, s);
    ...
    point2 = MathUtils.getPointOfStraightLine(startPoint, d, (c * (-1)));
  ...
  protected void createTrapezium(Point3D startPoint, Point3D endPoint) {
    ...
    point2 = MathUtils.getPointOfStraightLine(startPoint, v, (-1.0 * c));
