...
public class DXFGenerator implements Generator {
  ...
  public void generate(DraftDocument doc, Map<String, Object> context, OutputStream out)
    ...
    } else if (this.generationContext.hasAttribute(
        DXFGenerationContext.ATTRIBUTE_DEFAULT_PROFILE)) {
      profileName =
          this.generationContext
              .getAttribute(DXFGenerationContext.ATTRIBUTE_DEFAULT_PROFILE)
              .toString();
    }

    if (this.generationContext.getDXFGeneratorManager().hasDXFProfile(profileName)) {
      DXFProfile profile =
          this.generationContext.getDXFGeneratorManager().getDXFProfile(profileName);
  ...
  protected void outputSection(
      String section, DXFProfile profile, DXFOutput output, DraftDocument doc)
      throws GenerationException {

    if (this.generationContext.getDXFGeneratorManager().hasDXFSectionGenerator(section)
        && profile.hasDXFType(section)) {
      DXFSectionGenerator generator =
          this.generationContext.getDXFGeneratorManager().getDXFSectionGenerator(section);
