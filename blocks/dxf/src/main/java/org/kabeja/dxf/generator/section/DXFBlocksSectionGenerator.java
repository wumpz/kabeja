...
public class DXFBlocksSectionGenerator implements DXFSectionGenerator {
      ...
      for (DXFSubType subType : type.getDXFSubTypes()) {
        ...
        if (subType.getName().equals("AcDbBlockEntry")) {
          ...
          for (DraftEntity entity : block.getEntities()) {
            if (dxfContext
                .getDXFGeneratorManager()
                .hasDXFEntityGenerator(entity.getType().getHandle())) {
              DXFEntityGenerator entityGenerator =
                  dxfContext
                      .getDXFGeneratorManager()
                      .getDXFEntityGenerator(entity.getType().getHandle());
              if (profile.hasDXFType(entity.getType().getHandle())) {
                entityGenerator.generate(
                    output, entity, dxfContext, profile.getDXFType(entity.getType().getHandle()));
              }
            }
          }
  ...
  protected void outputBlockEntry(
      int[] groupCodes, Block block, DXFOutput output, DXFGenerationContext context)
      throws GenerationException {
  ...
  protected void outputBlocksEndEntry(
      int[] groupCodes, Block block, DXFOutput output, DXFGenerationContext context)
      throws GenerationException {
    for (int i = 0; i < groupCodes.length; i++) {
      switch (groupCodes[i]) {
