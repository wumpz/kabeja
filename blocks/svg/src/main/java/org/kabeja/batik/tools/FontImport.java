...
public class FontImport {
  ...
  public void importFonts() {
    ...
    if (files.length > 0) {
      try {
        BufferedWriter out =
            new BufferedWriter(
                new OutputStreamWriter(new FileOutputStream(fontDescriptionFile, true)));
  ...
  /**
   * @param fontDescriptionFile The fontDescriptionFile to set.
   */
  ...
  private void importFont(File source, File dest, BufferedWriter out) throws IOException {
    ...
    String[] args =
        new String[] {source.getAbsolutePath(), "-id", shx, "-o", dest.getAbsolutePath()};
    SVGFont.main(args);
