...
public abstract class AbstractSAXSerializer extends AbstractSAXFilter implements SAXSerializer {
  ...
  /**
   * Override this if your Transcoder support more options.
   *
   * @param t
   */
  protected void setupTranscoder(Transcoder t) {
    ...
    transcoder.addTranscodingHint(
        ImageTranscoder.KEY_PIXEL_UNIT_TO_MILLIMETER, new Float(this.PIXEL_UNIT_TO_MM));

    if (this.width > 0.0) {
      transcoder.addTranscodingHint(ImageTranscoder.KEY_WIDTH, new Float(this.width));
    }

    if (this.height > 0.0) {
      transcoder.addTranscodingHint(ImageTranscoder.KEY_HEIGHT, new Float(this.height));
    }
  }
  ...
  protected float unitsToPixel(String size) {
    if (size.endsWith("px")) {
      ...
      return (Float.parseFloat(size.substring(0, size.length() - 2)) * INCH_TO_MM)
          / PIXEL_UNIT_TO_MM;
