...
public class JavaScriptEngine implements ScriptEngine {
  ...
  public void eval(DraftDocument doc, InputStream script) throws ScriptException {
    try {
      Context ctx = Context.enter();
      Scriptable scope = ctx.initStandardObjects();
      Object jsOut = Context.javaToJS(doc, scope);
      ...
      Object result = ctx.evaluateReader(scope, new InputStreamReader(script), "<cmd>", 1, null);
