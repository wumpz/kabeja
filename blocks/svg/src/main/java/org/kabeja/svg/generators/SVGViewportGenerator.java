...
public class SVGViewportGenerator extends AbstractSVGSAXGenerator {
  ...
  public void toSAX(
    ...
    if (viewport.getViewportStatus() > 0) {
      ...
      SVGUtils.addAttribute(
          attr,
          SVGConstants.SVG_ATTRIBUTE_X,
          "" + SVGUtils.formatNumberAttribute((center.getX() - (viewport.getWidth() / 2))));
      SVGUtils.addAttribute(
          attr,
          SVGConstants.SVG_ATTRIBUTE_Y,
          "" + SVGUtils.formatNumberAttribute((center.getY() - (viewport.getHeight() / 2))));
      SVGUtils.addAttribute(
          attr,
      ...
      if (!viewport.isModelSpace() && !viewport.getViewportID().equals("1")) {
        ...
        // first the clip path
        SVGUtils.addAttribute(
            attr, SVGConstants.XML_ID, SVGUtils.validateID(viewport.getID() + "_clip"));
        ...
        SVGUtils.addAttribute(attr, SVGConstants.SVG_ATTRIBUTE_VIEWBOX, buf.toString());
        ...
        attr = new AttributesImpl();
        SVGUtils.addAttribute(attr, SVGConstants.XML_ID, SVGUtils.toValidateID(viewport.getID()));
  ...
  protected void layerToSAX(
      ContentHandler handler, Layer layer, Map context, Bounds viewBounds, Viewport viewport)
      throws SAXException {
    AttributesImpl attr = new AttributesImpl();
