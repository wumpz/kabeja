...
public class SVGViewUIComponent
    implements DraftDocumentViewComponent,
        Serviceable,
        DraftDocumentChangeListener,
        org.kabeja.svg.action.CanvasUpdateManager,
        PropertiesListener {
  ...
  String[] data =
      new String[] {
        "Modelspace-Calculated", "Modelspace", "Paperspace Calculated", "Paperspace", "Mixed"
      };
  ...
  public JComponent getView() {
    if (!this.initialized) {
      ...
      this.infoLabel =
          new JLabel("DXF2SVGViewer", JLabel.CENTER) {
      ...
      this.canvas.addSVGDocumentLoaderListener(
      ...
      this.canvas.addGVTTreeBuilderListener(
      ...
      this.canvas.addGVTTreeRendererListener(
          ...
          new MouseMotionAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
              if (count == 3) {
                ...
                SVGMatrix matrix =
                    canvas.getSVGDocument().getRootElement().getScreenCTM().inverse();
                double x = (matrix.getA() * p.getX()) + (matrix.getC() * p.getY()) + matrix.getE();
                double y = (matrix.getB() * p.getX()) + (matrix.getD() * p.getY()) + matrix.getF();
  ...
  public void propertiesChanged(Map properties) {
    if (properties.containsKey(SVGGenerator.PROPERTY_DOCUMENT_BOUNDS_RULE) && (this.doc != null)) {
      this.properties.put(
