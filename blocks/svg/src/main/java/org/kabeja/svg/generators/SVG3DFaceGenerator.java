...
public class SVG3DFaceGenerator extends AbstractSVGSAXGenerator {
  ...
  public void toSAX(
    ...
    if ((doc.getHeader().hasVariable("$SPLFRAME")
            && (doc.getHeader().getVariable("$SPLFRAME").getIntegerValue("70") == 1))
        || (face.getFlags() == 0)) {
      ...
      if ((flag & 1) == 0) {
        edgeToSAX(handler, face.getPoint1(), face.getPoint2(), svgContext, face);
      }

      if ((flag & 2) == 0) {
        edgeToSAX(handler, face.getPoint2(), face.getPoint3(), svgContext, face);
      }

      if ((flag & 4) == 0) {
        edgeToSAX(handler, face.getPoint3(), face.getPoint4(), svgContext, face);
      }

      if ((flag & 8) == 0) {
        edgeToSAX(handler, face.getPoint4(), face.getPoint1(), svgContext, face);
      }
    }
  }

  protected void edgeToSAX(
      ContentHandler handler, Point3D p1, Point3D p2, Map svgContext, Face3D face)
      throws SAXException {
    AttributesImpl attr = new AttributesImpl();
    // set the attributes
    SVGUtils.addAttribute(
        attr, SVGConstants.SVG_ATTRIBUTE_X1, SVGUtils.formatNumberAttribute(p1.getX()));

    SVGUtils.addAttribute(
        attr, SVGConstants.SVG_ATTRIBUTE_Y1, SVGUtils.formatNumberAttribute(p1.getY()));

    SVGUtils.addAttribute(
        attr, SVGConstants.SVG_ATTRIBUTE_X2, SVGUtils.formatNumberAttribute(p2.getX()));

    SVGUtils.addAttribute(
        attr, SVGConstants.SVG_ATTRIBUTE_Y2, SVGUtils.formatNumberAttribute(p2.getY()));
    super.setCommonAttributes(attr, svgContext, face);
