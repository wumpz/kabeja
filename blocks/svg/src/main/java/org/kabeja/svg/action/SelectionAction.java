...
public class SelectionAction extends AbstractAction
  ...
  public SelectionAction() {
    super(
        "Selection",
        new ImageIcon(UIUtils.resourceToBytes(SelectionAction.class, "/icons/cut_edit.gif")));
  ...
  public void handleEvent(Event evt) {
    if (this.state) {
      ...
      if (evt.getType().equals(SVGConstants.SVGEVENT_CLICK) && selecting) {
        ...
        // evt.stopPropagation();
      } else if (evt.getType().equals(SVGConstants.SVGEVENT_MOUSEDOWN) && !selecting) {
        // setup the start and end positions
        this.matrix =
            ((SVGDocument) this.selectionRectangle.getOwnerDocument())
                .getRootElement()
                .getScreenCTM()
                .inverse();
        this.startX =
            (matrix.getA() * event.getClientX())
                + (matrix.getC() * event.getClientY())
                + matrix.getE();
        this.startY =
            (matrix.getB() * event.getClientX())
                + (matrix.getD() * event.getClientY())
                + matrix.getF();
        ...
        // evt.stopPropagation();
      } else if (evt.getType().equals(SVGConstants.SVGEVENT_MOUSEMOVE) && selecting) {
        // evt.getTarget()
      ...
      } else if (evt.getType().equals(SVGConstants.SVGEVENT_MOUSEUP) && selecting) {
        selecting = false;
        process = true;
        this.changePosition(this.startX, this.startY, event.getClientX(), event.getClientY());
  ...
  protected void changePosition(float x, float y, float endX, float endY) {
    ...
    float width = (float) (((matrix.getA() * endX) + (matrix.getC() * endY) + matrix.getE()) - x);
    float height = (float) (((matrix.getB() * endX) + (matrix.getD() * endY) + matrix.getF()) - y);

    if (width < 0) {
      this.selectionRectangle.setAttributeNS(
          null, "x", "" + SVGUtils.formatNumberAttribute(x + width));
      this.selectionRectangle.setAttributeNS(
          null, "width", "" + SVGUtils.formatNumberAttribute(Math.abs(width)));
    } else {
      this.selectionRectangle.setAttributeNS(null, "x", "" + SVGUtils.formatNumberAttribute(x));
      this.selectionRectangle.setAttributeNS(
          null, "width", "" + SVGUtils.formatNumberAttribute(width));
    }

    if (height < 0) {
      this.selectionRectangle.setAttributeNS(
          null, "y", "" + SVGUtils.formatNumberAttribute(y + height));
      this.selectionRectangle.setAttributeNS(
          null, "height", "" + SVGUtils.formatNumberAttribute(Math.abs(height)));
    } else {
      this.selectionRectangle.setAttributeNS(null, "y", "" + y);
      this.selectionRectangle.setAttributeNS(
          null, "height", "" + SVGUtils.formatNumberAttribute(height));
    }
  }
  ...
  public void setDocument(SVGDocument doc) {
    ...
    rectangle.setAttributeNS(null, "x", SVGUtils.formatNumberAttribute((x - (width / 2))));

    rectangle.setAttributeNS(null, "y", SVGUtils.formatNumberAttribute(y - (height / 2)));

    rectangle.setAttributeNS(null, "width", SVGUtils.formatNumberAttribute(width * 2));
    rectangle.setAttributeNS(null, "height", SVGUtils.formatNumberAttribute(height * 2));
  ...
  protected void firePropertiesUpdated() {
    ...
    this.properties.put(BoundsFilter.PROPERTY_X, this.selectionRectangle.getAttribute("x"));
    this.properties.put(
        BoundsFilter.PROPERTY_Y,
        "" + (((-1) * Double.parseDouble(this.selectionRectangle.getAttribute("y"))) - height));
    this.properties.put(BoundsFilter.PROPERTY_WIDTH, this.selectionRectangle.getAttribute("width"));
    this.properties.put(BoundsFilter.PROPERTY_HEIGHT, "" + height);
    this.properties.put(BoundsFilter.PROPERTY_PROCESS, Boolean.toString(process));
