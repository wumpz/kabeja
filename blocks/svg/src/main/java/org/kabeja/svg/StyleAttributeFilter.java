...
public class StyleAttributeFilter extends AbstractSAXFilter {
  ...
  public static final String PROPERTY_DEFAULT_COLOR = "style.default.color";
  ...
  public void startElement(String uri, String localName, String name, Attributes atts)
    ...
    for (int i = 0; i < atts.getLength(); i++) {
      if (atts.getLocalName(i).equals(SVGConstants.SVG_ATTRIBUTE_FILL)) {
        fill = true;
      } else if (atts.getLocalName(i).equals(SVGConstants.SVG_ATTRIBUTE_STROKE_WITDH)) {
      ...
      if (atts.getLocalName(i).equals(SVGConstants.SVG_ATTRIBUTE_COLOR)) {
        colorFound = true;
    ...
    if (fill
        || localName.equals(SVGConstants.SVG_TEXT)
        || localName.equals(SVGConstants.SVG_TSPAN)) {
    ...
    if (buf.length() > 0) {
      SVGUtils.addAttribute(attributes, SVGConstants.SVG_ATTRIBUTE_STYLE, buf.toString());
    }
