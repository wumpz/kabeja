...
import org.kabeja.entities.util.HatchBoundaryLoop.BoundaryPathType;
...
public class SVGHatchGenerator extends AbstractSVGSAXGenerator {
    ...
    if (hatchBounds.isValid()) {
      ...
      if (hatch.isSolid()) {
        ...
        SVGUtils.addAttribute(
            attr, SVGConstants.SVG_ATTRIBUTE_FILL, SVGConstants.SVG_ATTRIBUTE_VALUE_CURRENTCOLOR);
        ...
        SVGUtils.addAttribute(attr, SVGConstants.XML_ID, SVGUtils.toValidateID(hatch.getID()));
        ...
        if (hatch.getHatchStyle() < 2) {
          ...
          SVGUtils.addAttribute(
              ...
              "url(#" + SVGUtils.toValidateID(hatch.getID()) + "-clip)");
        }
        ...
        HatchPattern pattern = hatch.getDocument().getHatchPattern(hatch.getHatchPatternID());

        attr = new AttributesImpl();
        SVGUtils.addAttribute(
            attr,
            SVGConstants.SVG_ATTRIBUTE_CLIP_PATH,
            "url(#" + SVGUtils.toValidateID(hatch.getID()) + ")");
        SVGUtils.startElement(handler, SVGConstants.SVG_GROUP, attr);
        ...
        convertHatchPatternToSAX(
            handler, svgContext, hatchBounds, hatch, transformContext, pattern);
        SVGUtils.endElement(handler, SVGConstants.SVG_GROUP);
      }
    }
  }

  protected void islandToClipPath(
      ContentHandler handler, Hatch hatch, SVGSAXGeneratorManager manager) throws SAXException {
    AttributesImpl attr = new AttributesImpl();
    SVGUtils.addAttribute(
        attr, SVGConstants.XML_ID, SVGUtils.toValidateID(hatch.getID()) + "-clip");
    SVGUtils.startElement(handler, SVGConstants.SVG_CLIPPING_PATH, attr);
    ...
    for (HatchBoundaryLoop loop : hatch.getBoundaryLoops()) {
      ...
      if (!loop.isBoundaryPathType(BoundaryPathType.OUTERMOST)) {
  ...
  protected void outermostToSVGPath(
      ContentHandler handler, Hatch hatch, SVGSAXGeneratorManager manager) throws SAXException {
  ...
  protected void loopToSVGPath(
      ContentHandler handler, HatchBoundaryLoop loop, SVGSAXGeneratorManager manager)
      throws SAXException {
    ...
    if (i.hasNext()) {
      ...
      String d =
          manager.getSVGPathBoundaryGenerator(entity.getType().getHandle()).getSVGPath(entity);
      ...
      while (i.hasNext()) {
        ...
        SVGPathBoundaryGenerator part =
            manager.getSVGPathBoundaryGenerator(entity.getType().getHandle());
        buf.append(' ');
   ...
   * java.util.Map)
   */
  public void convertHatchPatternToSAX(
    ...
    while (i.hasNext()) {
      ...
      SVGUtils.addAttribute(
          attr, "d", convertPatternToSVGPath(hatchBounds, hatch, pattern, dotLength));
  ...
  /**
   * Creates a SVG path of the used line family and fill the complete given bounds.
   *
   ...
   * @return
   */
  protected String convertPatternToSVGPath(
      Bounds b, Hatch hatch, HatchLineFamily pattern, double dotlength) {
    StringBuffer buf = new StringBuffer();
