...
public class SVGLeaderGenerator extends AbstractSVGSAXGenerator {
  public void toSAX(
    ...
    if (!leader.isSplinePath()) {
      ...
      // the used DIMSTYLE
      DimensionStyle style = leader.getDocument().getDimensionStyle(leader.getStyleNameID());
      ...
      if (leader.isArrowEnabled() && (style != null)) {
        ...
        if (style.hasProperty(DimensionStyle.PROPERTY_DIMLDRBLK)) {
          ...
          if (leader.getCoordinateCount() > 1) {
            ...
            double arrowLength =
                style.getDoubleProperty(DimensionStyle.PROPERTY_DIMASZ, 0.0)
                    * style.getDoubleProperty(DimensionStyle.PROPERTY_DIMSCALE, 1.0);

            if (length > (2 * arrowLength)) {
              double angle =
                  Math.toDegrees(
                      MathUtils.getAngle(
                          MathUtils.getVector(p1, p2), Constants.DEFAULT_X_AXIS_VECTOR));
