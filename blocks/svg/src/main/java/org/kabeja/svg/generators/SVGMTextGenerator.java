...
public class SVGMTextGenerator extends AbstractSVGSAXGenerator {
  public void toSAX(
    ...
    if ((mText.getTextStyle().length() > 0)
        && ((style = mText.getDocument().getStyle(mText.getTextStyle())) != null)) {
      if (style.isBackward()) {
    ...
    if (mText.getObliqueAngle() != 0.0) {
      ...
      transform.append(
          SVGUtils.formatNumberAttribute(
              alignmentPoint.getY() * Math.tan(Math.toRadians(1 * mText.getObliqueAngle()))));
      transform.append(')');
    }
    ...
    if (mText.getReferenceWidth() > 0.0) {
      SVGUtils.addAttribute(
          attr, SVGConstants.SVG_ATTRIBUTE_TEXT_LENGTH, "" + mText.getReferenceWidth());
    }
