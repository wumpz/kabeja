...
public class SVGGenerator extends AbstractSAXGenerator {
  ...
  /**
   * This property defines the way of calculation/setup the bounds of the Document. Possible values
   * are:
   *
   * <ul>
   *   <li>kabeja: Bounds calculate on the geometries. (default)
   *   <li>paperspace: extracts the values of the limits from paperspace
   *   <li>modelspace: extracts the values of the limits from modelspace
   * </ul>
   */
  ...
  public static final String PROPERTY_DOCUMENT_BOUNDS_RULE_PAPERSPACE_LIMITS_VALUE =
      "Paperspace-Limits";

  public static final String PROPERTY_DOCUMENT_BOUNDS_RULE_MODELSPACE_VALUE = "Modelspace";

  public static final String PROPERTY_DOCUMENT_BOUNDS_RULE_MODELSPACE_LIMITS_VALUE =
      "Modelspace-Limits";
  ...
  protected void setupProperties() {
    ...
    if (this.properties.containsKey(PROPERTY_OVERFLOW)) {
      this.overflow =
          Boolean.valueOf((String) this.properties.get(PROPERTY_OVERFLOW)).booleanValue();
    }
    ...
    if (this.properties.containsKey(PROPERTY_DOCUMENT_OUTPUT_STYLE)) {
      String value =
          ((String) this.properties.get(PROPERTY_DOCUMENT_OUTPUT_STYLE)).trim().toLowerCase();
      ...
      if (this.properties.containsKey(PROPERTY_DOCUMENT_OUTPUT_STYLE_NAME)) {
        this.outputStyleName =
            ((String) this.properties.get(PROPERTY_DOCUMENT_OUTPUT_STYLE_NAME)).trim();
      }
    }
  ...
  private void generateSAX() throws SAXException {
    try {
      ...
      if (this.outputStyle == PROPERTY_DOCUMENT_OUTPUT_STYLE_NOLAYOUT) {
        if (this.properties.containsKey(PROPERTY_WIDTH)) {
          SVGUtils.addAttribute(
              attr, SVGConstants.SVG_ATTRIBUTE_WIDTH, (String) this.properties.get(PROPERTY_WIDTH));
        }
        ...
        Dictionary dict =
            (Dictionary)
                this.doc.getRootDictionary().getObjectByName(Constants.DICTIONARY_KEY_LAYOUT);

        if (dict != null) {
          Layout layout = (Layout) dict.getObjectByName(this.outputStyleName);
          ...
          if (layout != null) {
            ...
            if (paper.isValid() && (paper.getWidth() > 0) && (paper.getHeight() > 0)) {
              SVGUtils.addAttribute(
                  attr, SVGConstants.SVG_ATTRIBUTE_HEIGHT, "" + paper.getHeight() + units);
              SVGUtils.addAttribute(
                  attr, SVGConstants.SVG_ATTRIBUTE_WIDTH, "" + paper.getWidth() + units);
            }
            ...
            if (b.isValid() && (b.getWidth() > 0) && (b.getHeight() > 0)) {
              bounds = b;
            }
          }
        }
      ...
      viewport =
          SVGUtils.formatNumberAttribute(bounds.getMinimumX())
              + " "
      ...
      attr.addAttribute(
          ...
          XMLConstants.KABEJA_NAMESPACE_PREFIX,
      ...
      if (this.context.containsKey(SVGContext.LINE_WIDTH)) {
        ...
        SVGUtils.addAttribute(
            ...
            SVGUtils.lineWidthToStrokeWidth((LineWidth) this.context.get(SVGContext.LINE_WIDTH)));
      } else {
        double sw =
      ...
      if (this.useModelSpaceBlock) {
        Block block = this.doc.getBlock(Constants.BLOCK_MODELSPACE);
        this.blockToSAX(block, null);
      } else if (this.usePaperSpaceBlock) {
        Block block = this.doc.getBlock(Constants.BLOCK_PAPERSPACE);
        this.blockToSAX(block, null);
      } else {

        // the layers as container g-elements
        Iterator<Layer> i = Utils.sortedLayersByZIndexIterator(this.doc.getLayers().iterator());
        while (i.hasNext()) {
  ...
  protected void blockToSAX(Block block, TransformContext transformContext) throws SAXException {
    ...
    SVGUtils.addAttribute(attr, SVGConstants.XML_ID, SVGUtils.toValidateID(block.getID()));
  ...
  protected Bounds getBounds() {
    ...
    if (this.boundsRule == PROPERTY_DOCUMENT_BOUNDS_RULE_PAPERSPACE) {
      ...
      if (this.doc.getHeader().hasVariable(Constants.HEADER_VARIABLE_PEXTMAX)
          && this.doc.getHeader().hasVariable(Constants.HEADER_VARIABLE_PEXTMIN)
          && useLimits) {
        Variable min = this.doc.getHeader().getVariable(Constants.HEADER_VARIABLE_PEXTMIN);
        Variable max = this.doc.getHeader().getVariable(Constants.HEADER_VARIABLE_PEXTMAX);
      ...
      if ((!bounds.isValid() || (bounds.getWidth() == 0.0) || (bounds.getHeight() == 0.0))
          && this.doc.getHeader().hasVariable(Constants.HEADER_VARIABLE_PLIMMIN)
          && this.doc.getHeader().hasVariable(Constants.HEADER_VARIABLE_PLIMMAX)
          && useLimits) {
        Variable min = this.doc.getHeader().getVariable(Constants.HEADER_VARIABLE_PLIMMIN);
        Variable max = this.doc.getHeader().getVariable(Constants.HEADER_VARIABLE_PLIMMAX);
      ...
      if (this.doc.getHeader().hasVariable(Constants.HEADER_VARIABLE_EXTMIN)
          && this.doc.getHeader().hasVariable(Constants.HEADER_VARIABLE_EXTMAX)
          && useLimits) {
        Variable min = this.doc.getHeader().getVariable(Constants.HEADER_VARIABLE_EXTMIN);
        Variable max = this.doc.getHeader().getVariable(Constants.HEADER_VARIABLE_EXTMAX);
      ...
      if ((!bounds.isValid() || (bounds.getWidth() == 0.0) || (bounds.getHeight() == 0.0))
          && this.doc.getHeader().hasVariable(Constants.HEADER_VARIABLE_LIMMIN)
          && this.doc.getHeader().hasVariable(Constants.HEADER_VARIABLE_LIMMAX)
          && useLimits) {
        Variable min = this.doc.getHeader().getVariable(Constants.HEADER_VARIABLE_LIMMIN);
        Variable max = this.doc.getHeader().getVariable(Constants.HEADER_VARIABLE_LIMMAX);
  ...
  protected void layerToSAX(Layer layer, boolean onModelspace) throws SAXException {
    AttributesImpl attr = new AttributesImpl();
    ...
    SVGUtils.addAttribute(
        attr, SVGConstants.SVG_ATTRIBUTE_COLOR, Color.getRGBString(Math.abs(layer.getColor())));
    SVGUtils.addAttribute(
        attr, SVGConstants.SVG_ATTRIBUTE_STROKE, SVGConstants.SVG_ATTRIBUTE_VALUE_CURRENTCOLOR);

    SVGUtils.addAttribute(
        attr, SVGConstants.SVG_ATTRIBUTE_FILL, SVGConstants.SVG_ATTRIBUTE_FILL_VALUE_NONE);

    if (!layer.isVisible() && onModelspace) {
      SVGUtils.addAttribute(
          attr,
          SVGConstants.SVG_ATTRIBUTE_VISIBILITY,
    ...
    // the stroke-width
    LineWidth lw = new LineWidth();
    if ((lineWeight > 0) && !context.containsKey(SVGContext.DRAFT_STROKE_WIDTH_IGNORE)) {
      lw.setType(LineWidth.TYPE_LINE_WEIGHT);
