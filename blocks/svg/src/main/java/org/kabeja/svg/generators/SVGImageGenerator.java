...
public class SVGImageGenerator extends AbstractSVGSAXGenerator {
  public void toSAX(
    ...
    ImageDefObject imageDef =
        (ImageDefObject) image.getDocument().getObjectByID(image.getImageDefObjectID());
    ...
    if (imageDef != null) {
      ...
      SVGUtils.addAttribute(
          attr, SVGConstants.SVG_ATTRIBUTE_WIDTH, SVGUtils.formatNumberAttribute(+imageSizeAlongU));
      SVGUtils.addAttribute(
          attr,
      ...
      transform.append(
          SVGUtils.formatNumberAttribute((insertPoint.getX() + (imageSizeAlongU / 2))));
      transform.append(SVGConstants.SVG_ATTRIBUTE_PATH_PLACEHOLDER);
      transform.append(
          SVGUtils.formatNumberAttribute((insertPoint.getY() + (imageSizeAlongV / 2))));
      transform.append(")");

      SVGUtils.addAttribute(attr, SVGConstants.SVG_ATTRIBUTE_TRANSFORM, transform.toString());
