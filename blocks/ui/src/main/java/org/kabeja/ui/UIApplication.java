...
public class UIApplication implements Application {
  ...
  public void start(Map properties) {
    try {
      ProcessingManager processorManager =
          SAXProcessingManagerBuilder.buildFromStream(
              ConfigHelper.getConfigAsStream(this.getClass().getClassLoader(), "conf/process.xml"));
      ServiceContainer sc =
          SAXServiceContainerBuilder.buildFromStream(new FileInputStream("conf/ui.xml"));
